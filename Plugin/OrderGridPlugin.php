...
class OrderGridPlugin
{
    ...
    protected function addCustomColumns(Collection $collection)
    {
        try {
            ...
            // Add order name from SHIPPING ADDRESS ONLY
     ...
     * Fixed to use shipping address first and last name
     */
    protected function addOrderNameColumn($select)
    {
        // Order Name from Shipping Address ONLY - Fixed expression
        $orderNameExpression = new \Zend_Db_Expr('
            TRIM(
                CONCAT(
                    IFNULL(shipping_addr.firstname, ""),
                    IF(
                        shipping_addr.firstname IS NOT NULL AND shipping_addr.lastname IS NOT NULL,
                        " ",
                        ""
                    ),
