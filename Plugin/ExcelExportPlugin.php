 ...
 * Order Name from Shipping Address ONLY
 *
 * @category    MagoArab
...
class ExcelExportPlugin
{
    ...
    private const REQUIRED_COLUMNS = [
        ...
        'Order Name' => ['Order Name', 'order_name', 'enhanced_customer_name'],
    ...
    {
        // First check if we already have order_name or enhanced_customer_name from the query
        $orderNameIndex = array_search('order_name', $header);
        if ($orderNameIndex !== false && isset($row[$orderNameIndex]) && !empty(trim($row[$orderNameIndex]))) {
            return trim($row[$orderNameIndex]);
        }
        
        $enhancedNameIndex = array_search('enhanced_customer_name', $header);
        if ($enhancedNameIndex !== false && isset($row[$enhancedNameIndex]) && !empty(trim($row[$enhancedNameIndex]))) {
            return trim($row[$enhancedNameIndex]);
        }
        
        // Get from shipping address fields directly
        $firstName = '';
        $lastName = '';
        
        // Check all possible shipping firstname fields
        $shippingFirstFields = ['shipping_firstname', 'Shipping First Name', 'Shipping Firstname'];
        ...
        // Check all possible shipping lastname fields  
        $shippingLastFields = ['shipping_lastname', 'Shipping Last Name', 'Shipping Lastname'];
        ...
        if (empty($firstName) && empty($lastName)) {
            ...
            if (!empty($orderId)) {
                $dbName = $this->getShippingNameFromDb($orderId);
                if (!empty($dbName)) {
                    return $dbName;
                }
            }
        ...
        // Return shipping name or empty (don't show Guest Customer in export)
        return !empty($fullName) ? $fullName : '';
    }
    
    /**
     * Get shipping name from database
     */
    protected function getShippingNameFromDb($orderId)
    {
        try {
            ...
            // Get shipping address from sales_order_address
            $query = $connection->select()
                ->from(['soa' => $resource->getTableName('sales_order_address')], ['firstname', 'lastname'])
                ->joinLeft(
                    ['so' => $resource->getTableName('sales_order')],
                    'so.entity_id = soa.parent_id',
                    []
                )
                ->where('so.increment_id = ? OR so.entity_id = ?', $orderId, $orderId)
