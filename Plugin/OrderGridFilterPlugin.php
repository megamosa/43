<?php
/**
 * MagoArab OrderEnhancer Filter Plugin - FIXED VERSION
 *
 * @category    MagoArab
...
class OrderGridFilterPlugin
{
    ...
    /**
     * Around addFieldToFilter to handle custom filters and fix ambiguous columns
     *
    ...
    {
        // Fix created_at ambiguous column issue
        if ($field === 'created_at' || (is_array($field) && in_array('created_at', $field))) {
            // Use main_table.created_at to avoid ambiguity
            return $proceed('main_table.created_at', $condition);
        }
        
        // Fix any other date fields that might be ambiguous
        if ($field === 'updated_at' || (is_array($field) && in_array('updated_at', $field))) {
            return $proceed('main_table.updated_at', $condition);
        }
        ...
        // For all other fields, proceed with the original method
    ...
    protected function ensureAmastyTableJoined(Collection $collection)
    {
        ...
        if (!isset($fromPart['amasty_custom_fields'])) {
            ...
            // Check if table exists before joining
            try {
                $connection = $collection->getConnection();
                ...
                $this->logger->warning('OrderGridFilterPlugin: Could not join Amasty table: ' . $e->getMessage());
            }
        }
    }
